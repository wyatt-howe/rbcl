...
class VerifyKey(encoding.Encodable, StringFixer, object):
    ...
    def __init__(self, key, encoder=encoding.RawEncoder):
        ...
        if len(key) != bcl.bindings.crypto_sign_PUBLICKEYBYTES:
            raise exc.ValueError(
                "The key must be exactly %s bytes long"
                % bcl.bindings.crypto_sign_PUBLICKEYBYTES,
            )
    ...
    def verify(self, smessage, signature=None, encoder=encoding.RawEncoder):
        ...
        :class:`~bcl.signing.BadSignatureError`.
        ...
        if signature is not None:
            ...
            if not isinstance(signature, bytes):
                raise exc.TypeError(
                    ...
                    % bcl.bindings.crypto_sign_BYTES,
                )

            if len(signature) != bcl.bindings.crypto_sign_BYTES:
                raise exc.ValueError(
        ...
        return bcl.bindings.crypto_sign_open(smessage, self._key)

    def to_curve25519_public_key(self):
        """
        Converts a :class:`~bcl.signing.VerifyKey` to a
        :class:`~bcl.public.PublicKey`

        :rtype: :class:`~bcl.public.PublicKey`
        """
        raw_pk = bcl.bindings.crypto_sign_ed25519_pk_to_curve25519(self._key)
...
class SigningKey(encoding.Encodable, StringFixer, object):
    ...
    value can be passed into the :class:`~bcl.signing.SigningKey` as a
    :func:`bytes` whose length is 32.

    .. warning:: This **must** be protected and remain secret. Anyone who knows
        the value of your :class:`~bcl.signing.SigningKey` or it's seed can
        masquerade as you.

    :param seed: [:class:`bytes`] Random 32-byte value (i.e. private key)
    :param encoder: A class that is able to decode the seed

    :ivar: verify_key: [:class:`~bcl.signing.VerifyKey`] The verify
    ...
    def __init__(self, seed, encoder=encoding.RawEncoder):
        ...
        if len(seed) != bcl.bindings.crypto_sign_SEEDBYTES:
            raise exc.ValueError(
                "The seed must be exactly %d bytes long"
                % bcl.bindings.crypto_sign_SEEDBYTES
            )

        public_key, secret_key = bcl.bindings.crypto_sign_seed_keypair(seed)
    ...
    def generate(cls):
        """
        Generates a random :class:`~bcl.signing.SigningKey` object.

        :rtype: :class:`~bcl.signing.SigningKey`
        """
        return cls(
            random(bcl.bindings.crypto_sign_SEEDBYTES),
    ...
    def sign(self, message, encoder=encoding.RawEncoder):
        ...
        :rtype: :class:`~bcl.signing.SignedMessage`
        """
        raw_signed = bcl.bindings.crypto_sign(message, self._signing_key)

        crypto_sign_BYTES = bcl.bindings.crypto_sign_BYTES
    ...
    def to_curve25519_private_key(self):
        """
        Converts a :class:`~bcl.signing.SigningKey` to a
        :class:`~bcl.public.PrivateKey`

        :rtype: :class:`~bcl.public.PrivateKey`
        """
        sk = self._signing_key
        raw_private = bcl.bindings.crypto_sign_ed25519_sk_to_curve25519(sk)
