...
from rbcl.utils import EncryptedMessage, StringFixer, random


class PublicKey(encoding.Encodable, StringFixer, object):
    """
    The public key counterpart to an Curve25519 :class:`rbcl.public.PrivateKey`
    ...
    SIZE = rbcl.bindings.crypto_box_PUBLICKEYBYTES
...
class PrivateKey(encoding.Encodable, StringFixer, object):
    ...
    .. warning:: This **must** be protected and remain secret. Anyone who
        knows the value of your :class:`~rbcl.public.PrivateKey` can decrypt
    ...
    SIZE = rbcl.bindings.crypto_box_SECRETKEYBYTES
    SEED_SIZE = rbcl.bindings.crypto_box_SEEDBYTES
    ...
    def __init__(self, private_key, encoder=encoding.RawEncoder):
        ...
        raw_public_key = rbcl.bindings.crypto_scalarmult_base(private_key)
    ...
    def from_seed(cls, seed, encoder=encoding.RawEncoder):
        ...
        raw_pk, raw_sk = rbcl.bindings.crypto_box_seed_keypair(seed)
    ...
    def generate(cls):
        """
        Generates a random :class:`~rbcl.public.PrivateKey` object
...
class Box(encoding.Encodable, StringFixer, object):
    ...
    The ciphertexts generated by :class:`~rbcl.public.Box` include a 16
    ...
    :param private_key: :class:`~rbcl.public.PrivateKey` used to encrypt and
        decrypt messages
    :param public_key: :class:`~rbcl.public.PublicKey` used to encrypt and
        decrypt messages

    :cvar NONCE_SIZE: The size that the nonce is required to be.
    """

    NONCE_SIZE = rbcl.bindings.crypto_box_NONCEBYTES
    ...
    def __init__(self, private_key, public_key):
        if private_key and public_key:
            ...
            self._shared_key = rbcl.bindings.crypto_box_beforenm(
    ...
    def encrypt(self, plaintext, nonce=None, encoder=encoding.RawEncoder):
        ...
        :rtype: [:class:`rbcl.utils.EncryptedMessage`]
        """
        if nonce is None:
        ...
        ciphertext = rbcl.bindings.crypto_box_afternm(
            plaintext,
            nonce,
            self._shared_key,
        )
    ...
    def decrypt(self, ciphertext, nonce=None, encoder=encoding.RawEncoder):
        ...
        plaintext = rbcl.bindings.crypto_box_open_afternm(
            ciphertext,
            nonce,
            self._shared_key,
        )
...
class SealedBox(encoding.Encodable, StringFixer, object):
    ...
    The ciphertexts generated by :class:`~rbcl.public.SecretBox` include
    the public part of the ephemeral key before the :class:`~rbcl.public.Box`
    ciphertext.

    :param public_key: :class:`~rbcl.public.PublicKey` used to encrypt
        messages and derive nonces
    :param private_key: :class:`~rbcl.public.PrivateKey` used to decrypt
        messages
    ...
    def encrypt(self, plaintext, encoder=encoding.RawEncoder):
        ...
        ciphertext = rbcl.bindings.crypto_box_seal(plaintext, self._public_key)
    ...
    def decrypt(self, ciphertext, encoder=encoding.RawEncoder):
        ...
        plaintext = rbcl.bindings.crypto_box_seal_open(
            ciphertext,
            self._public_key,
            self._private_key,
        )
